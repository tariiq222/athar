...
describe('OriginGuard', () => {
  ...
  beforeEach(() => {
    process.env = { ...ORIGINAL_ENV, CORS_ORIGINS: 'https://app.athar.sa,http://localhost:3000' };
  });
  afterAll(() => {
  ...
  it('rejects POST from Origin not in allow-list', () => {
    expect(() => new OriginGuard().canActivate(ctx('POST', 'https://evil.example'))).toThrow(
      ForbiddenException,
    );
  });
  ...
  it('rejects PATCH and DELETE from disallowed Origin', () => {
    expect(() => new OriginGuard().canActivate(ctx('PATCH', 'https://evil.example'))).toThrow(
      ForbiddenException,
    );
    expect(() => new OriginGuard().canActivate(ctx('DELETE', 'https://evil.example'))).toThrow(
      ForbiddenException,
    );
  });
